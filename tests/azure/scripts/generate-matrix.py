...
with open(args.azure_template) as f:
    ...
    for vm in data['vms']:
        ...
        for job_id, vm_job in enumerate(vm_jobs, 1):
            ...
            jobs[f'dyndb_ldap_tests_env_name_{job_id}'] = vm_job['container_job']
            jobs[f'dyndb_ldap_tests_to_run_{job_id}'] = ' '.join(vm_job['tests'])
            jobs[f'dyndb_ldap_tests_to_ignore_{job_id}'] = ' '.join(
                vm_job.get('ignore', ''))
            jobs[f'dyndb_ldap_tests_type_{job_id}'] = vm_job.get(
                'type', 'integration')
            ...
            jobs[f'dyndb_ldap_tests_replicas_{job_id}'] = replicas
            jobs[f'dyndb_ldap_tests_clients_{job_id}'] = clients
